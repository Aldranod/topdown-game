...
func update_dash_cooldown(timer: float, duration: float) -> void:
	if not dash_cooldown_indicator:
		return
	if timer > 0:
		if fade_tween and fade_tween.is_running():
			fade_tween.kill()
		dash_cooldown_indicator.visible = true
		dash_cooldown_indicator.modulate.a = 1.0 # Reset alpha to full
		var progress_percent = (timer / duration) * 100
		dash_cooldown_indicator.value = progress_percent
	else:
		if dash_cooldown_indicator.visible and (not fade_tween or not fade_tween.is_running()):
			fade_tween = create_tween()
			fade_tween.tween_property(dash_cooldown_indicator, "modulate:a", 0.0, 5)
			fade_tween.tween_callback(func():
				dash_cooldown_indicator.visible = false
				dash_cooldown_indicator.modulate.a = 1.0 # Reset alpha for next time
			)		
