...
func _ready() -> void:
	...
	PauseMenu.preview_stats_chaged.connect( _on_preview_stats_changed)
...
func _on_preview_stats_changed(item : ItemData) -> void:
	...
	update_chage_label(label_attack_change, attack_delta)
	update_chage_label(label_defense_change, defense_delta)		
	
	pass	

func update_chage_label(label : Label, value : int) -> void:
	if value > 0:
