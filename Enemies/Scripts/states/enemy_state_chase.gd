...
@export var turn_rate: float = 0.25
@export var can_charge : bool = false
@export var shoot_through_walls : bool = true  # Match your idle state setting
@onready var charge_ray: RayCast2D = $"../../ChargeRay"
@export_category("AI")
@export var vision_area : VisionArea
@export var attack_area : HurtBox
@export var min_charge_distance: float = 50.0
@export var state_aggro_duration : float = 3.0
@export var state_patience_duration: float = 10.0
@export var idle_state : EnemyState
@export var telegraph_state: EnemyStateTelegraph
@onready var attack_state: EnemyStateAttack = $"../Attack"
...
func process(_delta: float) -> EnemyState:
	...
	# Melee takes priority over everything
	if enemy._in_attack_range():
		return attack_state

	# Shoot check — either wall-ignoring (distance only) or requires line of sight
	if shoot_through_walls:
		if _player_in_shoot_range():
			return $"../Shoot"
	else:
		if enemy.can_shoot() and _check_line_of_sight():
			return $"../Shoot"

	# Vision and patience logic
	if vision_area.get_overlapping_bodies().has(enemy.player):
		_aggro_timer = state_aggro_duration
		...
		if has_los:
			...
			if dist > min_charge_distance and can_charge:
				return telegraph_state
		else:
			# Has vision overlap but no direct LOS — keep chasing, drain patience
			_patience_timer -= _delta
			if _patience_timer <= 0:
				return idle_state
	else:
		# Fully outside vision area — drain aggro timer
		_aggro_timer -= _delta
		if _aggro_timer <= 0:
			return idle_state

	# Keep moving toward player's last known position
	_direction = lerp(_direction, pathfinder.move_dir, turn_rate)
	enemy.velocity = _direction * chase_speed
	if enemy.set_direction(_direction):
		enemy.update_animation(anim_name)
...
# Distance-only check, ignores all walls and voids
func _player_in_shoot_range() -> bool:
	if not is_instance_valid(enemy.player):
		return false
	var dist = enemy.global_position.distance_to(enemy.player.global_position)
	var range_limit : float = enemy.shoot_range if "shoot_range" in enemy else 300.0
	return dist <= range_limit and enemy.can_shoot()

func _check_line_of_sight() -> bool:
	los_ray.target_position = enemy.to_local(enemy.player.global_position)
